...
/*
 * Simple example application that demonstrates how the palette API in
 * axoverlay is used.
 */
...
static gdouble
index2cairo(gint index)
{
  return ((index << 4) + index) / PALETTE_VALUE_RANGE;
}
                   ...
                   gpointer user_data)
{
  ...
  /* Clear background */
  gdouble val = index2cairo(0);
  cairo_set_source_rgba(cr, val, val, val, val);
...
main(int argc, char **argv)
{
  ...
  if (!setup_palette_color(0, 0, 0, 0, 0) ||
    ...
    printf("Failed to setup palette colors\n");
    return 1;
  }
